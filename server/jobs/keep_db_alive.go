...
func KeepDBAlive(ctx context.Context, db *sql.DB) {
	...
	runQuery := func() {
		ctxQ, cancel := context.WithTimeout(ctx, 10*time.Second)
		defer cancel()
		_, err := db.ExecContext(ctxQ, `INSERT INTO keepalive (pinged_at) VALUES (NOW())`)
		if err != nil {
			log.Printf("[cron] DB keepalive insert failed: %v\n", err)
			return
		}
		// Prune rows older than 30 days
		_, err = db.ExecContext(ctxQ, `DELETE FROM keepalive WHERE pinged_at < NOW() - INTERVAL '30 days'`)
		if err != nil {
